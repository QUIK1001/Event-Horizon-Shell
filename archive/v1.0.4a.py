...
def check_updates():
    try:
        ...
        if version.parse(latest_version) > version.parse(shell_version):
            print(Fore.RED + f"Update! {shell_version} < {latest_version}" + Style.RESET_ALL)
            print(Fore.CYAN + "Download: https://github.com/QUIK1001/Event-Horizon" + Style.RESET_ALL)
            return True
        else:
...
def calc():
    while True:
        try:
            ...
            if calc_act=="+":
                print(calc_num1, calc_act, calc_num2,Fore.GREEN + "equals> " + Style.RESET_ALL, calc_num1+calc_num2)
            elif calc_act=="-":
                print(calc_num1, calc_act, calc_num2,Fore.GREEN + "equals> " + Style.RESET_ALL, calc_num1-calc_num2)
            elif calc_act=="*":
                print(calc_num1, calc_act, calc_num2,Fore.GREEN + "equals> " + Style.RESET_ALL, calc_num1*calc_num2)
...
def shell():
    ...
    print(Fore.GREEN + "⣦ Type 'help' for commands, type 'exit' for exit\n" + Style.RESET_ALL)
    while True:
        cmd=input("#> ")
        if cmd=="help":
            ...
            try:
                ...
                print(Fore.GREEN + "folder", {shell_mkdir},"created in"  + Style.RESET_ALL, {shell_par_mkdir})
            except FileExistsError:
                print(Fore.RED + "⣏!⣽", {shell_mk_path},"already exists!" + Style.RESET_ALL)
        elif cmd=="rmdir":
            ...
            if not os.path.exists(shell_rm_path):
                ...
                print(Fore.GREEN + "folder", {shell_rm_path},"deleted successfully!" + Style.RESET_ALL)
        elif cmd=="time":
            shell_time=datetime.now()
            shell_for_time=shell_time.strftime(Fore.BLUE + "%Y-%m-%d %H:%M:%S:%MS" + Style.RESET_ALL)
            print(shell_for_time)
        elif cmd=="perf":
            clear_screen()
            print(Fore.RED + "⣦ System monitor started. Press Ctrl+C to stop." + Style.RESET_ALL)
            time.sleep(2.5)
            try:
                while True:
                    clear_screen()
                    print(Fore.RED + f"CPU: {psutil.cpu_percent()}% \nRAM: {psutil.virtual_memory().percent}%" + Style.RESET_ALL)
                    time.sleep(1)
...
def menu():
    event_horizon()
    while True:
        print(Style.BRIGHT + "1. calc\n2. echo\n3. ASCII arts\n4. shell info\n5. shutdown\n6. shell\n7. timer\n8. check updates" + Style.RESET_ALL)
